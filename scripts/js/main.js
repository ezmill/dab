...
function createFeedbackMaterial(){
    ...
    gTexture.minFilter = gTexture.magFilter = THREE.NearestFilter;
    ...
    // fbMaterial = new MultipassMaterial(renderer, scene, camera2, videoTexture, shaders);
    fbMaterial.init();
    ...
    screensaver.refractionPlaneMaterial.uniforms["refractionMap"].value = fbMaterial.fbos[2].renderTarget;
...
function draw() {
    ...
    // uniforms["mouse"].x = Math.sin(time);
    uniforms["mouse"].y = mouse.y;
    // uniforms["mouse"].y = Math.cos(time);
    uniforms["r2"] = r2;
